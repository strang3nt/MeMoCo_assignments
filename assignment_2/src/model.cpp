...
std::set<std::tuple<int, int>> Model::getFeasibleSolution(const int N, const std::vector<std::vector<double>>& graph, const int solLimit) {
  ...
  try {
    ...
    // solve
    CHECKED_CPX_CALL(CPXsetintparam, env, CPX_PARAM_INTSOLLIM, solLimit);
    ...
    for(int i = 0; i < N; ++i) {
      for(int j = 0; j < N; ++j)
      if(varVals[i * N + j] == 1) {
        feasibleSol.insert(LinKernighan::makeTuple(i, j));
      }
    }
  ...
  // if there is an exception, return empty set
  return feasibleSol;
}
...
Results Model::solveTsp(const int N, const std::vector<std::vector<double>>& graph) {
    ...
    gettimeofday(&tv2, NULL);
		double objval;
    CHECKED_CPX_CALL(CPXsetintparam, env, CPX_PARAM_INTSOLLIM, 1);
